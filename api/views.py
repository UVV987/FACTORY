...
def start_otk(request, department):
    if request.method == 'POST':
        model = get_model(department, request.POST.get('id'))
        dt = datetime.datetime.now()
...
def end_otk(request, department):
    if request.method == 'POST':
        model = get_model(department, request.POST.get('id'))
...
def give_qr(request, id, model):
    ...
    return render(request, 'qr.html', {'svg': ''.join(open(str(get_model(department, id).qr), 'r').readlines()).replace('svg', 'svg style="transform: scale(2);"')})
